defmodule Gate.Sector do
  ...
  defstruct [:name, :price_cop, :ttl_ms, :rows, :holds, :free, :next_hold_id, :expiry_queue]
  ...
  def reserve(sector, qty, :contiguous, now) do
    ...
    if sector.free < qty do
      ...
      result = 1..map_size(sector.rows)
      |> Enum.find_value(fn row_no -> row = sector.rows[row_no]
      ...
      case result do
        ...
        {row_no, new_row, seat_nos} ->
          ...
          new_expiry_queue = :queue.in({hold.expires_at, hold_id}, sector.expiry_queue)

          seat_ids = Enum.map(seat_nos, fn seat_no ->
            Gate.VenueSpec.seat_id(sector.name, row_no, seat_no)
          end)
  ...
  def reserve(sector, qty, :any, now) do
    ...
    if sector.free < qty do
      ...
      {new_rows, seats, remaining} = Enum.reduce_while(
        1..map_size(sector.rows),
        {sector.rows, [], qty},
        fn row_no, {rows, seats, remaining} ->
          row = rows[row_no]
      ...
      if remaining > 0 do
        ...
        seat_ids = Enum.map(seats, fn {row_no, seat_no} ->
          Gate.VenueSpec.seat_id(sector.name, row_no, seat_no)
        end)
    ...
    case Map.get(sector.holds, hold_id) do
      ...
      hold = %{state: :held} ->
        if now >= hold.expires_at do
          ...
          new_rows = sell_hold_seats(sector.rows, hold.seats)
          new_holds = Map.put(sector.holds, hold_id, %{hold | state: :confirmed})
          new_sector = %{sector | rows: new_rows, holds: new_holds}

          tickets = Enum.map(hold.seats, fn {row_no, seat_no} ->
            %{
              seat_id: Gate.VenueSpec.seat_id(sector.name, row_no, seat_no),
              sector: sector.name,
  ...
  @spec cancel(t, Gate.API.hold_id(), integer) ::
          {:ok, t}
          | {:error, :unknown_hold | :already_confirmed}
    ...
    case Map.get(sector.holds, hold_id) do
      ...
      hold = %{state: :held} ->
        ...
        new_sector = %{
          ...
          | rows: new_rows,
            holds: new_holds,
            free: sector.free + length(hold.seats)
        }
  ...
  def snapshot(sector, now) do
    ...
    snapshot = Enum.reduce(
      1..map_size(sector.rows),
      %{sold: [], held: [], free: [], revenue: 0},
      fn row_no, acc ->
        row = sector.rows[row_no]
        statuses = Gate.Row.by_status(row)

        sold_ids = Enum.map(statuses.sold, fn seat_no ->
          Gate.VenueSpec.seat_id(sector.name, row_no, seat_no)
        end)

        held_ids = Enum.map(statuses.held, fn seat_no ->
          Gate.VenueSpec.seat_id(sector.name, row_no, seat_no)
        end)

        free_ids = Enum.map(statuses.free, fn seat_no ->
          Gate.VenueSpec.seat_id(sector.name, row_no, seat_no)
        end)

        %{
          sold: acc.sold ++ sold_ids,
          held: acc.held ++ held_ids,
          free: acc.free ++ free_ids,
          revenue: acc.revenue + length(sold_ids) * sector.price_cop
        }
      end
    )
  ...
  defp sell_hold_seats(rows, seats) do
    Enum.reduce(seats, rows, fn {row_no, seat_no}, current_rows ->
      ...
      new_row = Gate.Row.sell(row, [seat_no])
  ...
  defp expire_holds(sector, now) do
    expire_from_queue(sector, now)
  end

  defp expire_from_queue(sector, now) do
    case :queue.out(sector.expiry_queue) do

      {:empty, _queue} ->
        sector

      {{:value, {expires_at, hold_id}}, remaining_queue} ->

        if now < expires_at do
          sector
        else
          sector = %{sector | expiry_queue: remaining_queue}

          case Map.get(sector.holds, hold_id) do

            hold = %{state: :held} ->
              new_rows = release_hold_seats(sector.rows, hold.seats)
              new_holds = Map.put(sector.holds, hold_id, %{hold | state: :expired})
              ...
              expire_from_queue(new_sector, now)
